...
CREATE DATABASE IF NOT EXISTS `soft_uni_database` /*!40100 DEFAULT CHARACTER SET utf8 */;
...
select `employees`.`employee_id`, `employees`.`job_title`, `employees`.`address_id`, 
...
FROM
    ...
    `departments` ON `employees`.`department_id` = `departments`.`department_id`
WHERE
    `employees`.`hire_date` > '1999-01-01'
...
order by `employees`.`first_name` asc, `projects`.`name` asc
limit 5;
...
select employees.employee_id, employees.first_name, 
 IF(year(projects.start_date) > '2004', NULL, projects.name) AS 'project_name'
from employees
join employees_projects on employees.employee_id = employees_projects.employee_id
