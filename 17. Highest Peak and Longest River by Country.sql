...
FROM
    `mountains_countries`
        JOIN
    `mountains` ON `mountains_countries`.`mountain_id` = `mountains`.`id`
        JOIN
    `peaks` ON `peaks`.`mountain_id` = `mountains`.`id`
WHERE
    `mountains_countries`.`country_code` = 'BG'
...
SELECT 
    `countries`.`country_name`, `rivers`.`river_name`
FROM
    `countries`
        left JOIN
    ...
    `rivers` ON `countries_rivers`.`river_id` = `rivers`.`id`
		join 
        `continents` on `continents`.`continent_code`= `countries`.`continent_code`
...
FROM countries as c
LEFT JOIN rivers as r ON c.country_code = r.country_code
